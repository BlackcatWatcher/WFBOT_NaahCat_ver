...
namespace TRKS.WF.QQBot
{
    ...
    public static class WFResource
    {
        ...
        private static WFApi GetTranslateApi()
        {
            ...
            Task.WaitAll(
                ...
                Downloader.GetCache<RmRiven[]>($"{source}WF_RmRiven.json", rmrivens => api.RmRiven = rmrivens)
            );
    ...
    public class WFTranslator
    {
        ...
        private Translator rmRivenTranslator = new Translator();
        ...
        private void InitTranslators()
        {
            ...
            rmRivenTranslator.Clear();
            foreach (var rmriven in translateApi.RmRiven)
            {
                if (!dictTranslators.ContainsKey("RmRiven"))
                {
                    dictTranslators.TryAdd("RmRiven", new Translator());
                }
                dictTranslators["RmRiven"].AddEntry(rmriven.Abr, rmriven.Name);
                rmRivenTranslator.AddEntry(rmriven.Abr, rmriven.Name);
            }
        ...
        public string TranslateWeapon(string str)
        {
            var zhResults = translateApi.Dict.Where(dict => dict.Zh.Format() == str && (dict.Type == "Weapon" || dict.Type == "Zaw")).ToList();
            var sb = new StringBuilder();
            string tw = str;
            foreach (var zhResult in zhResults)
            {
        ...
        public string[] TranslateAbrP(string str)
        {
            String[] Abresult = { "", "", "", "" };
            if (str.IsNullOrEmpty())
            {
            ...
            var enResults = translateApi.RmRiven.Where(rmriven => rmriven.Abr == str).ToList();
            if (!enResults.Any())
            {
                Trace.WriteLine($"transabrp：" + "EN-CN无结果" + str, "rmriven");
                Abresult[0] = "Any";
                return Abresult;
            }
            else
            {
                foreach (var enResult in enResults)
                {
                    Abresult[0] = enResult.Pre;
                    Abresult[1] = enResult.Name;
                    Abresult[2] = enResult.Unit;
                    if (enResult.Pre.Equals("+"))
                    {
                        Abresult[3] = "-";
                    }
                    else
        ...
        public string TranslateAbrPCntoEn(string str)
        {
            if (str.IsNullOrEmpty())
            {
                return "Any";
            }
            var zhResults = translateApi.RmRiven.Where(rmriven => rmriven.Name.Format() == str).ToList();
            if (!zhResults.Any())
            {
                Trace.WriteLine($"transabrp：" + "CN-EN无结果"+ str , "rmriven");
                return "Any";
            }
            else
            {
