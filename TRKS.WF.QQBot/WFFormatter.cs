...
namespace TRKS.WF.QQBot
{
    public static class WFFormatter
    {
        ...
        public static string ToString(Kuva kuva)
        {
            ...
            sb.AppendLine($"[{kuva.node}] 还有 {time} 过期");
        ...
        public static string ToString(Arbitration ar)
        {
            ...
            sb.AppendLine($"[{ar.node}] 还有 {time} 过期");
        ...
        public static string ToString(List<RivenInfo> infos)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine($"这是 {weapon} 紫卡的 {infos.Count} 条卖家信息.");
            foreach (var info in infos)
            {
                ...
                sb.AppendLine($"- 价格: {info.item_Price}白金 ({info.item_ResetNum}洗)");
        ...
        public static string ToString(VoidTrader trader)
        {
            ...
            else
            {
                ...
                sb.Append($"虚空商人还在流浪, {time} 后 抵达{trader.location}");
            }
        ...
        public static string ToString(WMInfo info, bool withQR, bool isbuyer)
        {
            ...
            sb.AppendLine($"喏，这些都是{(isbuyer ? "收购" : "出售")} [{item.zh.item_name}] 的：");
            foreach (var order in info.payload.orders)
            {
                sb.AppendLine($"[{order.user.ingame_name}] - {order.user.status} ");
                sb.AppendLine($"数量:{order.quantity} 等级:{order.mod_rank} 售价:{order.platinum}p");
                if (withQR)
                {
                    sb.AppendLine(
        ...
        [Pure]
        public static string ToStringde(WMInfo info, bool withQR, bool isbuyer, string name)
        {
            var sb = new StringBuilder();

            sb.AppendLine($"喏，这些都是{(isbuyer ? "收购" : "出售")} [{name}] 的：");
            foreach (var order in info.payload.orders)
            {
                sb.AppendLine($"[{order.user.ingame_name}] - {order.user.status} ");
                sb.AppendLine($"数量:{order.quantity} 等级:{order.mod_rank} 售价:{order.platinum}p");
                ...
                {
                    sb.AppendLine(
                        $"- 快捷回复: /w {order.user.ingame_name} Hi! I want to {(isbuyer ? "sell" : "buy")}: {name} for {order.platinum} platinum. (warframe.market)");
                }
        ...
        public static string ToString(WMInfoEx info, bool withQR, bool isbuyer)
        {
            ...
            {
                sb.AppendLine($"{order.order_Type} {order.platinum} 白金 [{order.userName}] {order.status}");
                if (withQR)
                {
                    sb.AppendLine(
